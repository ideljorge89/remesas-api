...
use App\Form\Type\Util\LegacyFormUtil;
 ...
 */
class PolyFormListener extends ResizeFormListener
{
    /**
     ...
     */
    public function __construct(array $prototypes = array(), array $options = array(), $allowAdd = false, $allowDelete = false, $typeFieldName = '_type', $indexProperty = null)
    {
        ...
        foreach ($prototypes as $prototype) {
            ...
            $type_name       = $prototype->getConfig()->getType()->getBlockPrefix();
            if (null === $defaultType) {
                $defaultType = $type;
            }
            $typeKey = $type;
            if ($type instanceof FormTypeInterface) {
                $typeKey = LegacyFormUtil::isFullClassNameRequired() ? get_class($type) : $type->getBlockPrefix();
            }
            $this->typeMap[$typeKey] = $type;
            $this->typeMap[$type_name] = $type;
